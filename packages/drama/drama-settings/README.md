...
- **The component list is only as good as the inventory it reads** — the status comes from `pluginInventory`, which a deployment may not compose; the page then reports every package as 无法查询 rather than assuming anything. Even with an inventory, a package mounted under a different module specifier than the one listed reads as 清单里没有, and a preset row that is only conditionally enabled reads as 按条件加载, because the Loader owns that decision. When two compositions name the same package — two presets, or a preset and the host Loader — the row with a fiber phase answers, because the mounted composition is the one a session can run; the page shows one row per package without saying which composition supplied it.
