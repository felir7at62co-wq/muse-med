---
description: "Web GUI 的短剧设置：持久 `drama` 设置段（交付目录、剪映草稿根目录、交付规格、BGM 库、生图通道行）、编辑它的设置页，以及页面上那份只读组件清单；供短剧流水线的使用者与维护者使用。"
...
持有持久 `drama` 设置段、编辑它的 **设置 → 短剧**页，以及页面上那份只读的组件清单。设置段承载流水线读取的东西：交付与剪映草稿目录、交付规格、本地 BGM 库，以及计费生图通道的 `gpt-image-2` 目录行。Host 半把 schema 注册进设置服务；Client 半绑定该命名空间、渲染这个页面，并把被拒绝的写入报告出来，而不是显示成已保存。某个计费步骤要不要先问，仍由 agent 自己拿捏，因此这里不存任何档位。
...
Web GUI 跑在哪里，就把这一行挂在哪里。Host 半需要一个已组合的设置提供方；没有它时不注册任何东西，Client 半则把该命名空间报告为不可用。
...
| `jianyingDraftDir` | 本机剪映根目录 | 可编辑草稿写入的绝对根目录；留空表示默认值 |
| `deliverySpec` | `1440` × `2560`、`60` fps、`4.6` Mbps | 成片的分辨率、帧率与码率下限 |
| `bgmDir` | `E:\aa-manju\bgm` | 选曲按情绪检索的本地 BGM 库 |
| `imageStandardId` | 缺省 | 计费资产图通道从哪一行 `gpt-image-2` 购买，写该行自己的 `id`；缺省表示交给通道自己判断，而那只在账户目录里恰好一行时成立 |

### 设置页

**设置 → 短剧**里每个字段一组：两个目录；交付规格的四个数字；BGM 库；以及资产图生成通道。每组显示的都是解析后的值，路径框留空即表示使用 schema 默认值——占位符就是那个默认值。

**保存**把表单编译成一次原子的命名空间写入，并报告结果：`已保存。`；宿主保留了另一个设置段时给出失败行；某个交付规格框里不是数字时给出对应提示（这种情况一个请求都不发）。**恢复默认**用一次写入清掉全部字段，这正是把设置段交还给 schema 默认值的方式。已经等于默认值的字段保持清空状态，而不是存一份默认值的副本，因此用户层永远不会说出它并不表示的意思。设置为只读文档时两个按钮都禁用，并说明原因。
...
- [drama-gate](../../guard/drama-gate/README.zh.md) —— 拥有流水线硬规则的工具分发门禁；某个计费步骤要不要先问是 agent 的判断，不是存下来的档位。
...
- **两个目录是不透明字符串** —— 页面只校验交付规格框里是不是数字，对路径不做任何校验：不检查存在性、不做规范化、不展开环境变量。宿主上不存在的路径会按原样存下。
...
- **路径框留空表示「用默认值」，不是「空」** —— 清空并保存会移除覆盖，之后框里会重新显示默认值。这两个字段没有办法存下有意为空的字符串，这与它们的消费者对空值的处理一致。
- **组件清单的可信度取决于它所读的清单** —— 状态来自 `pluginInventory`，而部署未必组合它；那时页面对每个包都报无法查询，而不是做任何假设。即便有清单，如果某个包实际挂载的模块说明符与列出的不同，它会显示为「清单里没有」；只带条件的 preset 行显示为「按条件加载」，因为这个判断归 Loader。
