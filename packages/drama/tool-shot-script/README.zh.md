...
使用 `drama_shot` 校验导演格式脚本、预演打包时长或编译 matched JSON 与单集文件。长镜头、慢台词、旁白与心声可以通过，并附建议性警告。字段格式错误、已绑定资产未确认或信息不完整，以及超出调用方明确时长预算的包仍阻止编译。工具保留台词原文与说话人身份，不调用提供方。
...
| 方法 | 必填输入 | 结果 |
|---|---|---|
| `validate` | `script`；可选 `assets` | 只读镜头事实、失败与警告 |
| `preview` | `script`、`assets`、`max_submit_seconds` | 只读打包方案 |
