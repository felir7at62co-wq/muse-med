...
- **时间必须来自真实发声，用 `drama_render subtitles` 测**：逐镜把该镜**自己的**音轨（此时还没有任何 BGM 混进来）解码，用这一镜自己的噪声底推导门限（第 20 百分位 + 12dB，且不低于 -45dBFS）定出发声段，再让台词计划里的每条台词占住它所在的那一段；cue 时间 = 该镜在时间线上的起点 + 镜内偏移。**不做语音转写、不联网、不需要模型**，因为每镜说什么早就写在剧本里，缺的只有时间。台词文字始终用剧本原文，不改写、不省略；一镜检出的发声段少于台词条数时，先按各段时长分配台词、再在段内按有效字数估算，这些 cue 标 `estimated_within_run` 并逐镜写进 `warnings`，`speech_alignment` 留在 `not_checked`，必须试听后校正。已经用语音识别跑过这些镜时，把逐镜时间写成对齐文档传给 `alignment`：它只提供时间，字幕文字仍取剧本原文，识别文本对不上会直接失败。写出的 cue 还会检查时长、重叠、越界与阅读速度（>20 有效字/秒失败，>12 警告）。9 个有效字/秒只作为**草稿预估**，不得用于最终交付。
