...
- **A paid storyboard submission needs an official asset in the project it names.** `jubian_storyboard` `generate` is refused unless the call binds itself to one project — by `project_dir`, or by a `script_id` that some `project_config.json` under the workshop declares — and that project records an `official=true` asset: an entry in `assets_manifest.json`, or a completed `official_assets` stage in `pipeline_state.json` when no manifest exists. A call the gate cannot place is refused with the two ways to place it, rather than judged against whichever project happens to carry evidence. Asset generation itself is deliberately not gated by this rule, because it is the step that runs before any asset is official.
- **A new billed asset needs a reconcile of the project it names.** `jubian_video` `image_generate` is refused unless the project the call binds to, as above, holds `_probe/asset-reconcile.json` whose `ran_at` is at most 24 hours old and no more than five minutes ahead, whose `blocking` and `ignored_without_note` are empty, and whose `ready` is true. A timestamp with no zone is read as China Standard Time, the zone the pipeline's tool writes. The evidence comes from the pipeline's own read-only reconcile, which is what compares `assets_manifest.json` with the assets the Jubian project already has selected: the manifest records what this pipeline generated, not what the project contains, so a model reading only the manifest regenerates an asset that is already there. Only this method is gated — `erase_subtitle`, `upscale`, `confirm_casting`, and the read methods stay with their own rules.
- **Every project rule binds by the operation, never by a search.** The two rules above resolve one project from the call itself: an injected `projectRoot`, else the call's own `project_dir`, else the workshop child whose `project_config.json` declares the call's `script_id`. Nothing scans the workshop for whichever project would pass, because one project's evidence must not authorize another project's paid call.
...
| [`src/rules.ts`](src/rules.ts) | The pure evaluator: the rule order, the write-method, paid-submission, and asset-creation tables, workshop path classification, operation-project resolution, and the official-asset and reconcile evidence readers |
...
- **No workspace root means no file rules** — a session that states no working directory and a deployment that configures no fallback leave the content rule with no path to classify, so a `write` or `edit` is allowed. The two project rules refuse instead, because a paid call they cannot place is exactly the call they exist to stop; the refusal names `project_dir` and `script_id` as the way to place it. The idempotency rule and the retired-name rule are unaffected.
- **Official-asset evidence is per project, not per shot** — the gate proves that the project the call names records an official asset, not that the shots being submitted reference it. It also trusts the caller's own binding: a call naming project A while its `storyboard_id` belongs to project B is judged against A.
- **The reconcile report is not checked against its own project** — the gate reads `_probe/asset-reconcile.json` below the project the call names, but never compares the report's `script_id` with that project's `project_config.json`, so a report copied into the wrong project directory still counts.
