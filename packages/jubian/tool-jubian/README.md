...
- **There is no spend cap, budget check, or paid-write confirmation** — the row submits any well-formed write the caller asks for. Idempotency prevents a duplicate charge for one key; it does not prevent a first charge. A deployment that needs a ceiling or a human confirmation must add a `tools/pre-execute` policy of its own. The same holds for the library writes: `rename`, `create_folder` and `move` change what a person reads in the console, and the row's only guard is the sentence in the description telling a model to ask first.
