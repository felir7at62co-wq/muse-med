...
随附 Web 的 `standard`、`ptc` 与 `cordis` preset 包含[显式文件交付](../../client/ui-deliverables/README.zh.md#explicit-deliveries)。`minimal` preset 保留固定的双工具训练配置。随附的 `short-drama` preset 在安装可选的短剧包后提供会话范围内的制作工具；Desktop Host 提供共享的剧变服务。
