...
The shipped Web `standard`, `ptc`, and `cordis` presets include [explicit file delivery](../../client/ui-deliverables/README.md#explicit-deliveries). The `minimal` preset keeps its fixed two-tool training configuration. The shipped `short-drama` preset supplies session-scoped production tools when its optional drama packages are installed; the Desktop Host supplies the shared Jubian services.
