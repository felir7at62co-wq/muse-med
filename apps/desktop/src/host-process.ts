...
export class DesktopHostProcess {
  ...
  async start(): Promise<DesktopHostReady> {
    ...
    if (!(requestPipe instanceof Writable) || !(responsePipe instanceof Readable)) {
      child.kill('SIGTERM')
      throw new Error('muse-med host did not expose the required byte pipes and IPC channel')
    }
    this.child = child
    ...
    responsePipe.once('end', () => {
      try {
        ...
        this.fail(new Error('muse-med host response pipe ended'))
      } catch (error) {
        this.fail(errorOf(error, 'muse-med host response pipe failed'))
      }
    })
    ...
    this.exitPromise = new Promise<void>((resolve) => {
      child.once('close', (code) => {
        ...
        else this.fail(new Error(`muse-med host stopped${suffix}`))
        resolve()
      })
    })
  ...
  async fetch(request: Request): Promise<Response> {
    ...
    if (child === undefined || !child.connected || this.requestPipe === undefined) {
      throw new Error('muse-med host is unavailable')
    }
    if (this.nextStreamId > 0xffff_ffff) throw new Error('muse-med host exhausted its request stream ids')
    ...
    return new Promise<Response>((resolve, reject) => {
      ...
      const abort = (): void => {
        ...
        this.enqueueRequestFrame(encodeDesktopRequestCancel(streamId)).catch((pipeError: unknown) => {
          this.fail(errorOf(pipeError, 'muse-med request pipe failed'))
        })
      ...
      this.pumpRequest(streamId, request, hasBody).catch((error: unknown) => {
        this.failPending(streamId, errorOf(error, 'muse-med request upload failed'))
      })
    })
  }
  ...
  private async pumpRequest(streamId: number, request: Request, hasBody: boolean): Promise<void> {
    ...
    if (body === null) throw new Error('muse-med request body disappeared before upload')
  ...
  private enqueueRequestFrame(frame: Buffer): Promise<void> {
    const write = this.requestWriteTail.then(async () => {
      ...
      if (pipe === undefined || pipe.destroyed) throw new Error('muse-med host request pipe is unavailable')
  ...
  private send(message: DesktopHostCommand): void {
    const child = this.child
    if (child === undefined || !child.connected) throw new Error('muse-med host IPC is unavailable')
  ...
  private handleResponseFrame(frame: DesktopHostResponseFrame): void {
    ...
    if (pending === undefined) {
      if (frame.streamId >= this.nextStreamId) {
        throw new Error(`muse-med host responded for unknown stream ${String(frame.streamId)}`)
      }
      return
    }
    switch (frame.type) {
      case 'start': {
        if (pending.responseStarted) throw new Error(`muse-med host started stream ${String(frame.streamId)} twice`)
      ...
      case 'data': {
        ...
        if (!pending.responseStarted || controller === undefined) {
          throw new Error(`muse-med host sent body data before a body start for stream ${String(frame.streamId)}`)
        }
      ...
      case 'end':
        if (!pending.responseStarted) {
          throw new Error(`muse-med host ended stream ${String(frame.streamId)} before its response start`)
        }
  ...
  private cancelResponse(streamId: number, reason: unknown): void {
    ...
    this.enqueueRequestFrame(encodeDesktopRequestCancel(streamId)).catch((error: unknown) => {
      this.fail(errorOf(error, 'muse-med request pipe failed'))
    })
    this.finishPending(streamId, false)
  }
