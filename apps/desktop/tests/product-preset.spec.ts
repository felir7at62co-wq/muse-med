...
it('loads only the product production mode without discovering shipped or personal presets', async () => {
  ...
  try {
    ...
    expect((await ctx.agentPresets.list()).map(row => row.id)).toEqual(['short-drama-local'])
