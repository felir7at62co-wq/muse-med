...
The shell displays **muse-med** and uses `renderer/icon.png`, a square PNG converted from the supplied black-background white-spider artwork; electron-builder converts it to platform installer icons. The Windows executable is `muse-med.exe`; release artifacts use `muse-med-${version}-${os}-${arch}.${ext}` across packaging and upload validation. Package identifiers and update endpoints retain their DSH identities. Packaged muse-med uses `~/.muse` for sessions, settings, credentials, and plugins; `MUSE_MED_HOME` explicitly overrides that location, while an inherited `DSH_HOME` or `MUSE_HOME` does not select it, because those may name a shared harness home. Development retains its isolated launcher-managed home.
