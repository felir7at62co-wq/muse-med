...
The bundled Feishu bridge stays inert until `appId`/`appSecret` are entered in this product's settings section and the bridge is enabled. While it is disabled there is no QR registration, no cross-instance sync, no control server, and no read of another instance's sync directory; the platform's own application permissions and visibility scope still decide who can reach the bot.
