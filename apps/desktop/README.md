...
The shell displays **muse-med** and uses `renderer/icon.png`, a square PNG converted from the supplied black-background white-spider artwork; electron-builder converts it to platform installer icons. The Windows executable is `muse-med.exe`; release artifacts use `muse-med-${version}-${os}-${arch}.${ext}` across packaging and upload validation. Package identifiers and update endpoints retain their DSH identities. Packaged muse-med uses `~/.muse` for sessions, settings, credentials, and plugins, and keeps reading the pre-rename `~/.muse-med` while that directory exists and `~/.muse` does not, so installing a build that names the new home never strands an installed copy; `MUSE_MED_HOME` explicitly overrides that location, while an inherited `DSH_HOME` or `MUSE_HOME` does not select it, because those may name a shared harness home. Development retains its isolated launcher-managed home.
...
The Desktop Host composes drama settings and Jubian tools once for all desktop sessions, with an automatic CNY 4000 ceiling per Jubian `script_id` for priced calls; local settings can change the limit. It mounts the maintained drama skill package and exposes exactly five product presets: the default `short-drama`, Standard, PTC, Minimal, and Cordis. Thin, read-only adapters reuse four native coding compositions; unrelated shipped and personal presets remain excluded. The Windows package preparation builds a hash-locked Python, media dependency, FFmpeg, and Whisper payload; the release remains unqualified until complete installer and clean-machine checks pass. Public binary distribution also requires the corresponding-source review recorded in the media descriptor.
...
The [community source builder](../../third_party/plugins/README.md) supplies five required local tarball roots; missing inputs stop package preparation rather than falling back to registry binaries. Codex subscription, FFmpeg tools, Ponytail, and Lark are built-in profile bundles. The marketplace ships unmounted because its plugin injects `webServer`, which the portless desktop composition disables, so its catalogue is consumed through the `./catalog` export the product build adds to that package instead.

Application → Desktop Plugins lists the five bundled community packages separately from user-installed plugins, loads the online catalogue only when asked, filters locally, and hands a confirmed npm-backed entry to the existing desktop package transaction. GitHub- or tarball-only entries stay browsable with a repository link and no install action. Development mode is read-only for package changes, and a catalogue network failure neither downloads anything nor hides the bundled inventory.

The bundled Feishu bridge is unmounted until this product's own settings document turns it on: `feishu.enabled` in `settings.yaml` under the product home, an absent document or section meaning off. The patch ships that row `disabled` at the entry level — the Loader switch that stops the plugin code from running, which a `config.enabled` key is not — and the Host recomputes the effective value from that document when the backend starts. That switch is the only activation authority, and it is not sufficient on its own: the compatibility overlay of the community build adds three plugin-level controls to the row that default to `true`, and the patch and the gate layer both hold `enabled`, `autoRegistration`, and `crossInstanceSync` at `false`, so turning the product switch on mounts the bridge with QR app registration and cross-instance synchronization still closed; the bridge also resolves a `dsh-lark-bridge` section of that same document over its entry config, so a stored `enabled: false` there keeps it inert even when `feishu.enabled` is true. A default Desktop therefore performs no QR app registration, starts no sync layer or control server, and reads no other instance's sync directory, and credentials stay out of the patch and come from this product's settings namespace alone. The switch takes effect at the next backend start, and the platform's own application permissions and visibility scope still decide who can reach the bot.

The product loads bundled skills and its own `$DSH_HOME/skills`, without scanning existing DSH, `.agents`, or default project skill roots; explicit plugin skill registration remains available. On Windows, place each custom skill at `%USERPROFILE%\.muse\skills\<skill-name>\SKILL.md`; `MUSE_MED_HOME` selects another product home. The Host creates the skills directory at startup. Development uses its separate home described below. Short-drama, Standard, and PTC expose the skill tool. Drama skills are unpacked from ASAR, and the Host exposes their real `app.asar.unpacked` paths to external Python. Windows startup prepends `runtime/media/python` and `runtime/media/ffmpeg/bin` to the child search path and sets the local ASR model directory. The [media preparer](scripts/prepare-media-runtime.ts) verifies locked inputs, dependency imports, encoding, subtitle burning, draft media probing, and offline ASR before publishing its output; reuse verifies the complete file inventory and reruns that smoke. These checks on the build machine do not replace installation on a machine without development tools. The installer carries the official VC++ prerequisite, checks its installed version, and requests authorization before installing it; rejection or failure prevents successful completion and automatic launch. Redistribution requires the publisher's applicable Microsoft license, not merely the runtime's end-user license.
...
Packaging records the GitHub Releases update source for every target, unsigned included, so packaging needs no update origin. `DSH_DESKTOP_AUTO_UPDATE_ENV` selects `test` or `production` for the signed target's completion record and for the later COS upload; an absent value selects `test`. A signed target's packaging requires the test deployment's HTTPS origin in `DOWNLOAD_TEST_ORIGIN`, while the production origin remains `https://download.deepseek.com`. Upload additionally requires the selected deployment's COS bucket in `DOWNLOAD_TEST_COS_BUCKET` or `DOWNLOAD_PROD_COS_BUCKET`. The target path is `_/harness/desktop/stable/<target>/`, where `target` is `mac-arm64`, `mac-x64`, or `win-x64`.
...
This product publishes its own releases to GitHub Releases at `https://github.com/felir7at62co-wq/muse-med`, which is the update source packaged into `app-update.yml`. Attach the installer to a release whose tag is a semantic version, such as `v0.1.6-alpha.2`: electron-updater's GitHub provider skips every release whose tag is not one. A prerelease version selects its first prerelease identifier as the channel, and packaging states that channel in the `github` publish entry because electron-builder names the emitted metadata file from that entry alone (defaulting to `latest`) and applies the version-derived channel only to the `generic`, `s3`, and `spaces` providers. Without the explicit channel a prerelease would ship `latest.yml` while every installed client asked for `alpha.yml`, taking a 404 before falling back. An installed `0.1.6-alpha.2` client therefore reads `alpha.yml` from the newest semantic-version release and needs that channel metadata beside the installer in the same release. Create the release as a draft and publish it after its assets pass verification: the provider reads a feed that lists published releases only. The `upload:*` commands above keep targeting Tencent COS for the signed release stream.
...
The command requires `DSH_DESKTOP_APP_ID` and the normal build dependencies, including Python and Visual C++ build tools for native modules. Set `PYTHON` to the Python executable when it is absent from `PATH`. It writes the installer to `.desktop-build/targets/win-x64/unsigned-artifacts/`, records the GitHub Releases update source without a publisher name, strips signing credentials, and creates no release completion record. It does not require EV credentials or an update origin. The signed packaging and upload commands retain their release requirements.
...
A packaged application checks its target-specific release stream ten seconds after the main window opens; the localized **Check for Updates…** menu item triggers the same check manually. An available release opens one native confirmation dialog. Accepting it waits for an in-flight check, downloads the Desktop installer, verifies its Authenticode signature when the package records a publisher name, stops the dsh child, and hands installation plus restart to electron-updater. The next launch displays the local loading page while reconciling the version-bound runtime.

Packaging records the GitHub Releases source in the packaged `app-update.yml`: `provider: github` with this product's owner, repository, and update channel. The updater checks for updates only while that file exists, so every target now checks, including an unsigned Windows build. A package built without signing records no `publisherName`, and electron-updater then accepts a downloaded installer without verifying an Authenticode signature; the channel metadata's SHA-512 still rejects a corrupted download, but nothing establishes who published it. The `upload:*` commands keep writing channel metadata for the deployment selected by `DSH_DESKTOP_AUTO_UPDATE_ENV`. NSIS differential packages and the macOS ZIP target allow electron-updater to reuse unchanged blocks when the channel metadata carries a `blockMapSize`; the assisted NSIS installer built here writes its block map to the `.exe.blockmap` sidecar, and that mode returns no `blockMapSize`, so clients download the installer in full instead of a differential patch. The manually installed DMG is notarized without a blockmap because it is not a macOS updater payload. The runtime and shell still form one signed Desktop release. macOS signing and notarization credentials use electron-builder's standard environment; Windows EV signing uses the public certificate, validated SignTool, SafeNet container, and runner PIN described above. The required Desktop release environment selects the application and platform signature identities that the build verifies.
