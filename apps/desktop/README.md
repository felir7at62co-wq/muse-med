...
The Desktop Host composes drama settings and Jubian tools once for all desktop sessions, with an automatic CNY 4000 ceiling per Jubian `script_id` for priced calls; local settings can change the limit. It mounts the maintained drama skill package and exposes exactly four product presets: the default `short-drama-local`, Standard, PTC, and Minimal. Thin, read-only adapters reuse the three native coding compositions; unrelated shipped and personal presets remain excluded. Minimal exposes only the persistent shell, not inherited paid or media tools. The Windows package preparation builds a hash-locked Python, media dependency, FFmpeg, and Whisper payload; the release remains unqualified until complete installer and clean-machine checks pass. Public binary distribution also requires the corresponding-source review recorded in the media descriptor.
