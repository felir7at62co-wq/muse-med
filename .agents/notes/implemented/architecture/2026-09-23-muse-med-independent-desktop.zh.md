...
Desktop Host 只发现产品根目录，其中包含默认的 `short-drama-local` 及两个只读原生适配器：`standard` 和 `ptc`。适配器复用上游组合，不维护副本，也不改变名册发现 API。上游 `minimal` 组合未被适配：它把 shell 放在嵌套 group 内，而以这种方式嵌套的组合不会导入该 group 的行，该模式会挂载成没有唯一工具的形态。短剧预设合并维护中的短剧工具与生产流程规则，共享剧变服务留在 Host，并使用公开配乐索引。部署自带预设和用户现有 Web 预设均不修改。persona 要求当前用户明确授权制作；已有的自动单项目预算上限仍是独立执行策略，不是新增的授权机制。
...
Host 启用一个文件系统技能提供方，显式指定随包及产品 home 的 `skills` 根目录，并在启动时创建可写目录。默认根目录发现保持禁用。标准和 PTC 适配器禁用原生的近层文件系统提供方，让所有产品预设继承 Host 提供方，而不选择旧 `.agents` 或项目目录。短剧、标准和 PTC 模式提供技能工具。显式插件技能注册仍然可用。
...
准备阶段的私有冒烟插件等待 credentials Service 就绪，为三个产品预设各挂载两个并存 Agent，检查工具、随包技能及产品自定义技能，并用旧目录中的同名技能作负对照；未完成时会记录失败原因。父进程必须读到此完成记录；Host 就绪不代表插件已成功激活。启动限时 60 秒，冒烟检查不提供凭据，也不发送模型请求。
...
启动测试覆盖默认与显式产品 home、开发模式行为及 Windows 媒体环境传播。Loader 测试覆盖产品名册和可撤销的守卫行为；它不能替代完整预设激活、干净机器媒体执行、GUI 检查或安装包验证。抢本离线测试覆盖假账号 home 隔离及缺凭据时拒绝请求。两条渲染路径已用现有 FFmpeg 二进制通过真实汉字冒烟检查；新包尚未完成干净 Windows 验证。已有 DSH 账号与会话保留在原 home，当前 Web 安装仍可能显示原来的两个模式。
