...
Desktop Host 拥有 `presets/short-drama-local`，且只发现该产品根目录。预设合并维护中的短剧工具与生产流程规则，共享剧变服务留在 Host，并使用公开配乐索引。部署自带预设和用户现有 Web 预设均不修改。persona 要求当前用户明确授权制作；已有的自动单项目预算上限仍是独立执行策略，不是新增的授权机制。
...
Host 启用一个显式指定随包根目录的文件系统技能提供方，并禁用默认根目录发现。产品预设继承它，不注册会选中旧 `.agents` 或继承环境目录的近层提供方。显式插件技能注册仍然可用。
...
准备阶段的私有冒烟插件等待 credentials Service 就绪，挂载完整产品 Agent，检查工具和随包技能，并且只在 Agent 释放之后记录成功。父进程必须读到此完成记录；Host 就绪不代表插件已成功激活。启动限时 60 秒，冒烟检查不提供凭据，也不发送模型请求。
...
**增加预设别名或隐藏 ID。** 使用全新 home 的独立产品没有需要迁移的历史会话。单一生产模式无需扩展名册协议；现有 Web 安装的迁移仍是另一项工作。
