...
The same package is mounted by a surface this repository does not compose: the muse Web profile (`~/.muse/profiles/web`) lists the market original 0.6.1 in `dsh.profile.bundles`, with no `SOURCE.json` and none of the three plugin-level controls — that `Config` carries `appId`, `appSecret`, and `requireMention`, and its boot path starts the sync layer and QR onboarding unconditionally once the row starts. The only switch that keeps that row from running is the Loader's entry-level `disabled`, so the installed profile's own patch layer (`cordis.patch.yml`, applied after every bundle layer) disables `feishu-channel`. The Desktop gate layer reaches the Desktop composition alone, so the two surfaces are gated independently, and the Web-side entry lives in installed profile state rather than in this repository: a rebuilt profile, or a bundle list edited later, has to re-add it.
