...
The Desktop Host discovers only its product root, which holds five presets and defaults to `short-drama`: that product composition plus four read-only native adapters, `standard`, `ptc`, `minimal`, and `cordis`. The adapters reuse the upstream compositions without maintaining copies or changing roster discovery APIs, and each repoints the composition's own filesystem skill provider instead of leaving it to select deployment roots. A nested `cordis:group` does mount its rows through the adapter, so the `minimal` composition keeps the persistent shell that is its only tool. The short-drama preset combines maintained drama tools with the production workflow rules, keeps shared Jubian services on the Host, and uses the public BGM catalog. The product edits neither the deployment's shipped presets nor any user preset directory; the Web profile selects the shipped roster instead of the user root. The current user's explicit production authorization is required by the persona; the existing automatic per-project budget ceiling remains a separate execution policy, not a new authorization mechanism.

The product's own short-drama preset was renamed from `short-drama-local` to `short-drama`. A session created before that rename names the old id in its creation header and in every `agent-preset/selected` event, and resuming one failed with `agent-preset/not-found` while a root still answered to the new id, because this version has no alias mechanism. The roster therefore carries one resolution-only table of deprecated ids, mapping the old id to `short-drama`: it never joins `list()`, so the picker still shows exactly the roots' presets, and `resolve` prefers a real preset, so an archived directory restored under the old id keeps answering for itself. Delete the entry when no session created before the rename remains resumable and no settings document still stores the old id as its default.
...
The Host enables one filesystem skill provider with explicit bundled and product-home `skills` roots, and creates the writable directory during boot. Default-root discovery remains disabled. The Standard and PTC adapters disable the composition's nearer native filesystem provider, the Minimal composition carries none, and Cordis keeps its own row serving only the composition-authoring skills its persona names, so no product preset selects old `.agents` or project roots. Short-drama, Standard, PTC, and Cordis expose the skill tool. Explicit plugin skill registration remains available.
