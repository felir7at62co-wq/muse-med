...
Stage 1 has landed too: the sync is a procedure rather than a hope. `scripts/upstream-sync-triage.ts` classifies each conflict by the side that owns the decision, from what each side holds rather than from a list of filenames — `product` for a file the fork kept and the upstream ref deleted, `upstream` for the mirror image, `own` where both sides edited, and `generated` for a path a declared generator writes. On the current base that is 77 conflicts: product 14, upstream 0, own 57, generated 6, none unassigned. `scripts/upstream-sync-triage.spec.ts` covers the classification, the manifest, and the refusal, and [the upstream-sync cookbook](../../../../docs/cookbook/upstream-sync.md) carries the procedure, including the rule against hand-editing a generated file to express a product fact.
...
Stage 1's criterion is met: `pnpm exec tsx scripts/upstream-sync-triage.ts` assigns all 77 conflicts an owner with none unassigned, leaves the repository unchanged, and `pnpm exec vitest run scripts/upstream-sync-triage.spec.ts --maxWorkers=1` passes its 15 cases. What Stage 1 asked for beyond code — rehearsing on a schedule rather than behind a release, and not letting the gap outgrow one release channel — is discipline the cookbook states and no gate can enforce.
